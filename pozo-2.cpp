...
double  s[ (L_INTERVALS+2*KORD-3) * (KORD)],
        strace[(L_INTERVALS+2*KORD-3) * (KORD)],
...
static void interaccion(const double * __restrict__ const x, const double * __restrict__ const w){
  ...
  for(int k=KORD ; k<KORD+L_INTERVALS ; k++){
    ...
    for(int l=0 ; l<INT_G ; l++){
      ...
      bsplvb(KORD, rr2, k-1, &Sps[idx_dense(basek2, l, INT_G, KORD)], Sp_1);
    }
  }

  double *d_Sps=0;
  checkCudaErrors(cudaMalloc((void**)&d_Sps, 
    L_INTERVALS * INT_G * KORD * sizeof(d_Sps[0])));
  checkCudaErrors(cudaMemcpy(d_Sps, Sps, (size_t)
                              (L_INTERVALS * INT_G * KORD * sizeof(Sps[0])),
                              cudaMemcpyHostToDevice));

  double * d_f=0;
  const size_t f_size = (L_INTERVALS+2*KORD-3) * 2 * (KORD) * sizeof(d_f[0]);
  checkCudaErrors(cudaMalloc((void**)&d_f, f_size));
  double *d_x = 0; double * d_w = 0;
  checkCudaErrors(cudaMalloc((void**)&d_x, INT_G * sizeof(d_x[0])));
  checkCudaErrors(cudaMalloc((void**)&d_w, INT_G * sizeof(d_w[0])));

  checkCudaErrors(cudaMemcpy(d_x, x, (size_t)INT_G * sizeof(d_x[0]),
                              cudaMemcpyHostToDevice));
  checkCudaErrors(cudaMemcpy(d_w, w, (size_t)INT_G * sizeof(d_w[0]),
                              cudaMemcpyHostToDevice));

  double *d_vef = 0, *d_strace = 0;

  size_t size_vef = (L_INTERVALS+2*KORD-3) * 2 * (KORD) * (L_INTERVALS+2*KORD-3) * 2 * (KORD) * sizeof(double);
  checkCudaErrors(cudaMalloc((void**)&d_vef, size_vef));
  checkCudaErrors(cudaMemset(d_vef, 0, size_vef));

  for(int i=0 ; i< (L_INTERVALS+2*KORD-3) ; i++){
    strace[i] = s[idx(i, i, nb)];
  }

  checkCudaErrors(cudaMalloc((void**)&d_strace, (L_INTERVALS+2*KORD-3) * sizeof(double)));
  checkCudaErrors(cudaMemcpy(d_strace, strace, (L_INTERVALS+2*KORD-3)  * sizeof(double), cudaMemcpyHostToDevice));
  ...
  for(int i=KORD ; i<KORD+L_INTERVALS ; i++){
    ...
    for(int j=0 ; j<INT_G ; j++){
      ...
      call_integracion(rr1, w1, d_f, d_x, d_w, d_Sps, f_size);
      /*/*checkCudaErrors(cudaMemcpy(f, d_f, (size_t)f_size,
      ...
      call_sumatoria_vef(w1, d_f, d_Sps, basek, i, j, d_vef, d_strace);
  ...
  checkCudaErrors(cudaMemcpy(Vef, d_vef, size_vef, cudaMemcpyDeviceToHost));

  checkCudaErrors(cudaFree(d_Sps));
  checkCudaErrors(cudaFree(d_f));
  checkCudaErrors(cudaFree(d_x));
  checkCudaErrors(cudaFree(d_w));
  checkCudaErrors(cudaFree(d_vef));
  checkCudaErrors(cudaFree(d_strace));
    ...
    for(size_t n = 0 ; n < nb ; n++){
      for(size_t m = n ; m < nb; m++){
        ...
        for(size_t np=cnt2 ; np<nb &&  np < n + KORD; np++){
            int cnt = std::max(np, m < KORD ? 0 : m - KORD + 1) - np;
            indp += cnt;
...
int main(void) {
    ...
    gaulegm(x, w);
    t_fin = clock();
    diff_time = ((double) t_fin - t_in) / CLOCKS_PER_SEC;
    t_total += diff_time;
    printf("gaulegm: %.12f\n", diff_time);
